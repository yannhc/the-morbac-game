...
class Board
    ...
    def initialize
      ...
      # On crée les cases du morpion (9), comme demandé dans la ressource, et qui vont être ajoutées à l'array
    ...
    # La, c'est l'affichage du tableau
  def board_display
    puts " "
    puts "            | 1 | 2 | 3 "
    print "         "
  ...
  # On supprime les données du tableau pour une nouvelle partie
  def board_clean
    n = " "
  ...
  # Méthode qui sert à faire un tour au joueur
  def play_turn(player)

    # Change en X ou O suivant celui qui joue
    if player == "01"
      a = "X"
    else 
      a = "O"
    ...
    # Choix de la case
    puts "      Tu veux quelle case le moche ?"
    print "      > "
    input = gets.chomp
    loop do
      case input
      ...
      when "A1"
        # Si la case est vide, on a le symbole du joueur, mais si c'est déjà pris on renvoie à l'input
    
        if array_case[0].value != " "
          puts " "
          puts "      C'est déjà pris bigoudi !"
      ...
      when "C3"
        ...
        # Si écriture pas bonne, renvoi à l'input
        puts " "
        puts "      Hummmmm c'est pas bon là !!!"
        puts "      Mets bien les majuscules, merde alors! : "
        print "      > "
        input = gets.chomp
      end
    end
  ...
  def victory
    ...
    # Probabilités de victoires pour joueur 1 (X)
    ...
    # Probabilités de victoires pour joueur 2 (symbole O)
    ...
    # Egalité, si toutes les cases sont remplis et que ça ne mène pas à la victoire d'un joueur
